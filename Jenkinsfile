pipeline {
  ...
  stages{
    ...
    stage('Apply') {
      ...
      steps {
        sh "cd fitcycle_terraform/ && terraform init --backend-config=bucket=BUCKET_NAME --backend-config=key=terraform.tfstate --backend-config=region=us-east-1 -lockfile=false"
