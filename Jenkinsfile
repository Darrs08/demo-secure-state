pipeline {
  ...
  stages{
    ...
    stage('Apply') {
      ...
      steps {
        sh "cd fitcycle_terraform/ && terraform init --backend-config=bucket=demodars --backend-config=key=path/to/my/key/some.tfstate --backend-config=region=us-east-1 -lockfile=false && terraform apply --input=false --var-file=example_vars_files/us_west_1_mysql.tfvars --auto-approve"
      }
    }
  }
  post {
